//! Module-key dispatch for HUD pointer/hover/scroll/render routing.
//!
//! Each function here is a thin dispatcher that forwards to the concrete submodule
//! (`agent_list`, `conversation_list`, `info_bar`, `thread_pane`) based on the [`HudWidgetKey`].
//! Keeping the routing in its own file lets `mod.rs` be a curated re-export surface only, per
//! the crate's mod.rs-as-surface rule.
...
use super::{agent_list, conversation_list, info_bar, thread_pane};
