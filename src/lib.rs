//! The NeoZeus application as a library crate.
//!
//! Exposes one public entry point — `run()` — used by `src/main.rs` to launch the binary. Also
//! exposes `shared` for the `src/bin/neozeus-*` support binaries to consume the daemon wire
//! protocol and supporting helpers. Everything else is `mod`-private to the library crate.
