...
pub(super) struct FakeDaemonClient {
    ...
    pub(super) session_metadata: Mutex<
        std::collections::HashMap<String, crate::shared::daemon_wire::DaemonSessionMetadata>,
    >,
...
impl TerminalDaemonClient for FakeDaemonClient {
    ...
    fn create_session_with_env(
        ...
        *next += 1;
        ...
        let mut metadata = crate::shared::daemon_wire::DaemonSessionMetadata::default();
        for (key, value) in env_overrides {
            match key.as_str() {
                ...
                "NEOZEUS_AGENT_KIND" => {
                    metadata.agent_kind =
                        crate::shared::daemon_wire::DaemonAgentKind::from_env_name(value);
                }
                _ => {}
            }
        }
