...
mod tests {
    ...
    fn decodes_v1_session_list_payloads_without_created_order() {
        ...
        let message =
            read_server_message(&mut Cursor::new(framed)).expect("v1 payload should decode");
    ...
    fn session_list_wire_format_omits_created_order_for_v1_compatibility() {
        ...
        let DaemonResponse::SessionList { sessions } = response.expect("response should be ok")
        else {
